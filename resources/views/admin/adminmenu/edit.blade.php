    ...
    <div class="wrapper wrapper-content">
        <div class="row">
                ...
                <div class=" text-center middle-box "style="margin-top: 20px">
                    <form id="signupForm" method="post" action="{{route('menu.update',['id'=>$menu->id])}}" class="form-horizontal">
                        {{ csrf_field() }}
                        {{ method_field('put') }}
                    <div class="col-md-12">
                        <div class="form-group" >
                            ...
                            <div class="col-sm-9">
                                <input id="name" type="text" name="name"  class="form-control" placeholder="请输入文本" value="{{$menu->name}}">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input id="model" type="text" name="url"  class="form-control" placeholder="访问地址" value="{{$menu->url}}">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input id="model" type="text" name="order_id"  class="form-control" placeholder="访问地址" value="{{$menu->order_id}}">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="pid">
                                    ...
                                    @foreach($parent_list as $p)
                                        <option value="{{$p->id}}" @if($menu->pid==$p->id) selected @endif>{{$p->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="node_id">
                                    @foreach($node_list as $n)
                                        <option value="{{$n->id}}" {{ $menu->node_id==$n->id?'selected':'' }}>{{$n->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                ...
                                <label class="checkbox-inline">
                                    <input type="radio" name="status" value="{{ $k }}" required @if($menu->status==$k) checked @endif>{{ $status}}
                                </label>
                                @endforeach
                            </div>
                        </div>
