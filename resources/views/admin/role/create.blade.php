    ...
    <div class="wrapper wrapper-content">
        <div class="row">
                ...
                <div class=" text-center middle-box "style="margin-top: 20px">
                    <form id="signupForm" method="post" action="{{route('role.store')}}" class="form-horizontal">
                        {{ csrf_field() }}
                    <div class="col-md-12">
                            ...
                            <div class="col-sm-9 text-left">
                                ...
                                @foreach($node_list as $node)
                                    <div class="node">
                                        <div class="checkbox">
                                        <label>
                                            <input type="checkbox" name="node_id[]" value="{{ $node->id }}" required>{{ $node->name }}
                                        </label>
                                        </div>
                                        <div class="checkbox col-sm-offset-1">
                                            @foreach($action_arr as $key=>$action)
                                                <label style="width: 50px;">
                                                    <input class="node_action" type="checkbox" name="node_{{$key}}_{{$node->id}}" value="1" >{{ $action }}
                                                </label>
                                            @endforeach
                                        </div>
                                    </div>
