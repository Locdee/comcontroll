...
<body class="gray-bg">
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-sm-4">
            ...
            <ol class="breadcrumb">
                <li>
                    <a href="{{route('role.index')}}">浏览角色</a>
                </li>
                <li>
                    <strong>增加角色</strong>
                </li>
        ...
        <div class="col-sm-8">
            <div class="title-action">
                <a href="{{route('role.index')}}" class="btn btn-primary">返回</a>
            </div>
        </div>
    </div>
    ...
    <div class="wrapper wrapper-content">
        <div class="row">
                ...
                <div class=" text-center middle-box "style="margin-top: 20px">
                    <form id="signupForm" method="post" action="{{route('role.update',['id'=>$role->id])}}" class="form-horizontal">
                        {{ csrf_field() }}
                        {{ method_field('put') }}
                    <div class="col-md-12">
                        <div class="form-group" >
                            ...
                            <div class="col-sm-9">
                                <input id="name" type="text" name="name"  class="form-control" placeholder="请输入文本" value="{{ $role->name }}">
                                <span class="help-block m-b-none">角色名称</span>

                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label ">相关节点：</label>
                            <div class="col-sm-9 text-left">
                                ...
                                @foreach($node_list as $node)
                                    <div class="node">
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" name="node_id[]" value="{{ $node->id }}" @if($role_node=$role->node()->wherePivot('node_id',$node->id)->first()) checked @endif >{{ $node->name }}
                                            </label>
                                        </div>
                                        <div class="checkbox col-sm-offset-1">
                                            @foreach($action_arr as $key=>$action)
                                                <label style="width: 50px;">
                                                    <input class="node_action" type="checkbox" name="node_{{$key}}_{{$node->id}}" value="1"  @if(!empty($role_node) && $role_node->pivot->$key==1 ) checked @endif>{{ $action }}
                                                </label>
                                            @endforeach
                                        </div>
                                    </div>
...
//                alert($(this).parents(".node").find('.node_action:checked').length);
            ...
            $("#signupForm").validate({
                ...
                submitHandler:function(form){
                    Actionsubmit(form,'POST','{{route("role.index")}}');
                }
            });
