    ...
    <div class="wrapper wrapper-content">
        <div class="row">
                ...
                <div class=" text-center larger-box "style="margin-top: 20px">
                    <form id="signupForm" method="post" action="{{route('questionnaire.update',['id'=>$questionnair->id])}}" class="form-horizontal">
                        {{ csrf_field() }}
                        {{ method_field('put') }}
                    <div class="col-md-12">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="activity_id">
                                    @foreach($activity_list as $o)
                                        <option value="{{$o->id}}" {{$questionnair->activity_id==$o->id?'selected':''}}>{{$o->activityname}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <textarea name="question" style="width: 100%;height: 155px">{{$questionnair->question}}</textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="type">
                                    @foreach($type_arr as $key=>$i)
                                        <option value="{{$key}}" {{$questionnair->type==$key?'selected':''}}>{{$i}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div id="choice_block" style="background-color: lightyellow">
                            <div class="form-group" >
                                ...
                                <div class="col-sm-9 answer_block">
                                    <input id="name" type="text" name="anser_iterm_A"  class="form-control" placeholder="请输入文本" value="{{isset($questionnair->answer_iterm['A'])?$questionnair->answer_iterm['A']:''}}">
                                </div>
                            </div>
                            <div class="form-group" >
                                <label class="col-sm-3 control-label">选项B：</label>
                                <div class="col-sm-9 answer_block">
                                    <input id="name" type="text" name="anser_iterm_B"  class="form-control" placeholder="请输入文本"  value="{{isset($questionnair->answer_iterm['B'])?$questionnair->answer_iterm['B']:''}}">
                                </div>
                            </div>
                            <div class="form-group" >
                                <label class="col-sm-3 control-label">选项C：</label>
                                <div class="col-sm-9 answer_block">
                                    <input id="name" type="text" name="anser_iterm_C"  class="form-control" placeholder="请输入文本"  value="{{isset($questionnair->answer_iterm['C'])?$questionnair->answer_iterm['C']:''}}">
                                </div>
                            </div>
                            <div class="form-group" >
                                <label class="col-sm-3 control-label">选项D：</label>
                                <div class="col-sm-9 answer_block">
                                    <input id="name" type="text" name="anser_iterm_D"  class="form-control" placeholder="请输入文本"  value="{{isset($questionnair->answer_iterm['D'])?$questionnair->answer_iterm['D']:''}}">
                                </div>
                            </div>
                            <div class="form-group" >
                                <label class="col-sm-3 control-label">选项E：</label>
                                <div class="col-sm-9 answer_block">
                                    <input id="name" type="text" name="anser_iterm_E"  class="form-control" placeholder="请输入文本"  value="{{isset($questionnair->answer_iterm['E'])?$questionnair->answer_iterm['E']:''}}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group" >
                            ...
                            <div class="col-sm-9" id="choise_right_answer">
                                <input id="name" type="text" name="right_answer_choise"  class="form-control" placeholder="请输入文本" value="{{$questionnair->type!=3?$questionnair->right_answer:''}}">
                                <span class="help-block m-b-none">多选用英文逗号分隔</span>
                            </div>
                            <div class="col-sm-9 " id="judge_answer">
                                <label><input type="radio" name="right_answer_judge" value="right" {{$questionnair->right_answer=='right'?'checked':''}}>对</label>
                                <label><input type="radio" name="right_answer_judge" value="wrong" {{$questionnair->right_answer=='wrong'?'checked':''}}>错</label>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label">答案解析：</label>
                            <div class="col-sm-9">
                                <textarea name="explain" style="width: 100%;height: 155px">{{$questionnair->explain}}</textarea>
                            </div>
                        </div>
                        <div class="form-group" >
                            <label class="col-sm-3 control-label">得分：</label>
                            <div class="col-sm-9">
                                <input id="score" type="number" name="score"  class="form-control" placeholder="请输入得分" value="{{$questionnair->score}}">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <select name="status" class="form-control help-block m-b-none">
                                    @foreach($status_arr as $k=>$status)
                                        <option value="{{ $k }}" {{$questionnair->status==$k?'selected':''}}>{{$status}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
