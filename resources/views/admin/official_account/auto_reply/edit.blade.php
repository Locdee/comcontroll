    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-sm-8">
            <div class="title-action">
                <a href="{{route('auto_reply.index')}}" class="btn btn-primary">返回</a>
            </div>
        </div>
    </div>
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="official_account_id">
                                    @foreach($official_list as $o)
                                        <option value="{{$o->id}}" {{$auto_reply->official_account_id==$o->id }}>{{$o->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
//                return;
            ...
            $("#signupForm").validate({
                ...
                submitHandler:function(form){
                    Actionsubmit(form,'POST','{{route("auto_reply.index")}}');
                }
            });
