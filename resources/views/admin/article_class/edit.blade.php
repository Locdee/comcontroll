...
<body class="gray-bg">
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-sm-4">
            ...
            <ol class="breadcrumb">
                <li>
                    <a href="{{route('article_class.index')}}">浏览文章分类</a>
                </li>
                <li>
                    <strong>编辑</strong>
                </li>
    ...
    <div class="wrapper wrapper-content">
        <div class="row">
                ...
                <div class=" text-center middle-box "style="margin-top: 20px">
                    <form id="signupForm" method="post" action="{{route('article_class.update',['id'=>$article_class->id])}}" class="form-horizontal">
                        {{ csrf_field() }}
                        {{ method_field('put') }}
                    <div class="col-md-12">
                        <div class="form-group" >
                            ...
                            <div class="col-sm-9">
                                <input id="name" type="text" name="classname"  class="form-control" placeholder="请输入文本" value="{{$article_class->classname}}">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="official_account_id">
                                    @foreach($official_list as $o)
                                        <option value="{{$o->id}}" {{ $article_class->official_account_id==$o->id?'selected':'' }}>{{$o->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-9">
                                @foreach($status_arr as $k=>$status)
                                    <label class="checkbox-inline">
                                        <input type="radio" name="status" value="{{ $k }}" required {{$article_class->status==$k?'checked':''}}>{{ $status}}
                                    </label>
                                @endforeach
                            </div>
                        </div>
